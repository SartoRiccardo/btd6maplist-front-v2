...
watch(
  [leaderboardFormats, () => route.query["format"]],
  ([fmts, queryFormat]) => {
    if (!fmts.length) return;
    if (
      typeof queryFormat === "string" &&
      fmts.some((f) => f.id.toString() === queryFormat)
    ) {
...
function onFormatChange(key: string) {
  const defaultValue = FORMATS_WITH_POINTS.includes(Number(key))
    ? "points"
    : "lccs";
...
watch(
  [availableValues, () => route.query["value"]],
  ([values, queryValue]) => {
    if (
      typeof queryValue === "string" &&
      values.some((v) => v.key === queryValue)
    ) {
...
const currentValueOption = computed(
  () =>
    LEADERBOARD_VALUES.find((v) => v.key === selectedValue.value) ??
...
<template>
  <div>
    <h1
      class="text-center mt-6 mb-4 font-['Luckiest_Guy'] text-3xl md:text-4xl"
    >
      Leaderboard
    </h1>
