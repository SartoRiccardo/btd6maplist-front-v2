...
const canBan = computed(
  () =>
    auth.hasPermission(permissions.user.ban) &&
    auth.user?.discord_id !== userId.value,
);
...
<template>
  <div>
    ...
    <!-- Profile actions -->
    <div
      v-if="(canBan || canEditProfile) && user"
    ...
    >
      ...
      >
        <i
          :class="user.is_banned ? 'bi bi-arrow-clockwise' : 'bi bi-ban'"
          class="mr-1"
        />
    ...
    <!-- Completions -->
    <div class="my-6">
      <h2 class="font-['Luckiest_Guy'] text-2xl text-center mb-4">
        Completions
      </h2>
      <CompletionList
        :params="{
    ...
    <div class="my-6">
      ...
      <MapGrid
        :maps="mapsLoading ? undefined : createdMaps"
      ...
      <p
        v-if="!mapsLoading && createdMaps?.length === 0"
