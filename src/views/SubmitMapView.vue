...
const eligibleFormats = computed(() => {
  ...
  const allOpen = formatsResponse.value.data.filter(
    (f) => f.map_submission_status !== 'closed',
  );
  if (!maplistMap.value) return allOpen;
  const existingIds = getFormatsMapIsIn(maplistMap.value);
  return allOpen.filter((f) => !existingIds.includes(f.id));
});
...
watch(eligibleFormats, (fmts) => {
  if (fmts.length === 1 && formModel.value.format_id !== fmts[0]!.id) {
    ...
    formModel.value = { ...formModel.value, format_id: preselectedFormatId.value };
  }
}, { immediate: true });
...
<template>
  <div>
    ...
    <!-- Not authenticated -->
    <Panel v-if="!auth.isAuthenticated && !auth.isLoading" class="flex flex-col items-center gap-4">
      <p class="text-(--color-text-muted)">You must be logged in to submit a map.</p>
      <DiscordLoginButton />
    </Panel>
    ...
    <template v-else-if="!auth.isLoading">
      ...
      <Panel class="mb-6">
        ...
        <div class="mt-2 text-sm">
          <p v-if="nkLoading" class="text-(--color-text-muted)">
            <i class="bi bi-arrow-repeat animate-spin inline-block" /> Validating...
          </p>
          ...
          <template v-else-if="nkMap">
            ...
            <p v-if="maplistLoading" class="text-(--color-text-muted) mt-1">
              <i class="bi bi-arrow-repeat animate-spin inline-block" /> Checking list status...
            </p>
          </template>
        </div>
      </Panel>
      ...
      <template v-if="isDataReady">
        <!-- No eligible formats -->
        <Panel v-if="eligibleFormats.length === 0">
          <p class="text-(--color-text-muted) text-center">
            This map is already in all available lists, or no lists are currently accepting map submissions.
          </p>
        </Panel>
