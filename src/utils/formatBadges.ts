...
export function getMapFormatBadges(
  ...
  if (
    map.difficulty != null &&
    (!visible || visible.includes(FORMAT_EXPERT_LIST))
  ) {
    ...
    if (diff)
      badges.push({
        icon: diff.image,
        label: `${diff.name} Expert`,
        slug: fmt.slug,
      });
  }

  if (
    map.botb_difficulty != null &&
    (!visible || visible.includes(FORMAT_BEST_OF_THE_BEST))
  ) {
    const diff = BOTB_DIFFICULTIES.find((d) => {
      const v = d.value;
      return Array.isArray(v)
        ? v.includes(map.botb_difficulty!)
        : v === map.botb_difficulty;
    });
  ...
  if (map.retro_map && (!visible || visible.includes(FORMAT_NOSTALGIA_PACK))) {
    ...
    badges.push({
      icon: map.retro_map.preview_url,
      label: map.retro_map.name,
      slug: fmt.slug,
      squareImage: true,
    });
  }
