...
function buildMapsParams(params?: GetMapsParams): string {
  ...
  if (params.format_id != null)
    searchParams.set("format_id", params.format_id.toString());
  if (params.format_subfilter != null)
    searchParams.set("format_subfilter", params.format_subfilter.toString());
  ...
  if (params.verified_by != null)
    searchParams.set("verified_by", params.verified_by.toString());
  ...
  if (params.medal_formats != null)
    searchParams.set("medal_formats", params.medal_formats.toString());
...
function buildMapFormData(data: CreateMapRequest | UpdateMapRequest): FormData {
  ...
  if (data.r6_start_file != null)
    fd.append("r6_start_file", data.r6_start_file);
  if (data.map_preview_url != null)
    fd.append("map_preview_url", data.map_preview_url);
  if (data.custom_map_preview_file != null)
    fd.append("custom_map_preview_file", data.custom_map_preview_file);
  if (data.map_data != null) fd.append("map_data", data.map_data);
  if (data.map_notes != null) fd.append("map_notes", data.map_notes);
  if (data.placement_curver != null)
    fd.append("placement_curver", data.placement_curver.toString());
  if (data.placement_allver != null)
    fd.append("placement_allver", data.placement_allver.toString());
  if (data.difficulty != null)
    fd.append("difficulty", data.difficulty.toString());
  if (data.botb_difficulty != null)
    fd.append("botb_difficulty", data.botb_difficulty.toString());
  ...
  if (data.verifiers) {
    for (let i = 0; i < data.verifiers.length; i++) {
      ...
      fd.append(
        `verifiers[${i}][version]`,
        v.version != null ? v.version.toString() : "",
      );
    }
  }
