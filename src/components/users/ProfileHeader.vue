...
const medalTypes = [
  {
    ...
    tooltip: "CHIMPS Completions",
  },
  {
    key: "black_border" as const,
    image: "/images/medals/medal_bb.webp",
    tooltip: "CHIMPS Completions (Black Border)",
  },
  {
    key: "no_geraldo" as const,
    image: "/images/medals/medal_nogerry.webp",
    tooltip: "No Optimal Hero",
  },
  {
    key: "current_lcc" as const,
...
<template>
  <div class="bg-(--color-secondary) rounded-(--radius-panel) p-[5px]">
    ...
    <div
      ...
      :style="{
        backgroundImage: `url(${user.banner_url || DEFAULT_BANNER_URL})`,
      }"
    >
      <!-- Avatar -->
      <img
    ...
    <div
      v-if="
        user.medals && medalTypes.some((m) => (user.medals?.[m.key] ?? 0) > 0)
      "
      class="flex justify-center gap-3 pt-2"
    >
      <template v-for="medal in medalTypes" :key="medal.key">
        <Tooltip
          v-if="(user.medals?.[medal.key] ?? 0) > 0"
          :text="medal.tooltip"
        >
          <div class="relative">
            <img :src="medal.image" alt="" class="w-[60px] h-[60px]" />
            <p
              class="absolute bottom-[-0.5rem] right-[-0.2rem] mb-0 font-['Luckiest_Guy'] font-border text-xl"
            >
